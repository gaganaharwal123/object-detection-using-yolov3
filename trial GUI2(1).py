...
from yolo import live_feed,imag_det,vid_det
...
class ObjDet(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        for F in (StartPage, Viddet, livefeed, Imgdet):
...
class StartPage(tk.Frame): #initial page of the GUI
    def __init__(self, parent, controller):
        ...
        button = tk.Button(self, text="Detection on Video",command=lambda: controller.show_frame(Viddet))
        button.pack(pady=530,padx=110,side="right")
        
        button1 = tk.Button(self, text="Detection on Image",command=lambda: controller.show_frame(Imgdet)).place(x=290, y=670)
...
class Viddet(tk.Frame): #object detection based on video
    def __init__(self, parent, controller):
        val=1
        tk.Frame.__init__(self, parent)
        ...
        button2 = tk.Button(self, text="Detection on Image",command=lambda: controller.show_frame(Imgdet))
...
class livefeed(tk.Frame):
    def __init__(self, parent, controller):
        ...
        button1 = tk.Button(self, text="Detection on Image",command=lambda: controller.show_frame(Imgdet)).place(x=250, y=654)

        button2 = tk.Button(self, text="Detection on Video",
                            command=lambda: controller.show_frame(Viddet))
        button2.pack(pady=530,padx=130,side="right")
        
class Imgdet(tk.Frame):
    def __init__(self, parent, controller):
        val=3
        tk.Frame.__init__(self, parent)
        
        label = tk.Label(self, text="Object detection using YOLOv3 on an input image", font=LARGE_FONT)
        label.pack(pady=50,padx=50,side="top", fill="both", expand = True)
        
        tk.Label(self, text="Enter location of .jpg file").place(x=180, y=80)
        searched = tk.StringVar()
        search = tk.Entry(self, textvariable = searched).place(x=330, y=80)
        ...
        button1 = tk.Button(self, text="Detection on Video",command=lambda: controller.show_frame(Viddet)).place(x=250, y=654)
...
def check_class(person,car,bike,building,val,searched='live feed'):
    ...
    if(searched!='live feed'):
        text = searched.get()
    if(val==1):
        vid_det(text,object_det)
    elif(val==2):
        live_feed(object_det)
    elif(val==3):
        imag_det(text,object_det)
        img = cv.imread("image_out.jpg")
        cv.imshow("image", img)
